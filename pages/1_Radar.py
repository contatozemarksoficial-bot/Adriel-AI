...
def obter_banco_produtos():
    return [
        ...
        {"Product Name": "Java Burn", "Nicho do Produto": "Café Termogênico", "Melhor País Estratégico": "Austrália 🇦🇺", "CPC Médio Est. ($)": "$0.45", "Tendência / Veredito": "Poder de compra alto", "Afirmação Estratégica / Por que Anunciar": "VALIDADO: Poder de compra alto na Austrália. Oferta de conversão imediata misturada com café."},
...
if "dados_radar_dinamico" not in st.session_state:
    ...
    df_inicial.insert(2, "Tendência / Volume", ["🔥 SOBE (Alta Volume)", "🔥 SOBE (Alta Volume)", "🔥 SOBE (Alta Volume)", "🟢 ESTÁVEL"] + ["🔥 SOBE (Alta Volume)"] * 6 + ["🟢 ESTÁVEL"] * 12)
    st.session_state.dados_radar_dinamico = df_inicial

# BOTÃO DE ROTAÇÃO E ATUALIZAÇÃO DO RADAR (ALGORITMO DE TENDÊNCIAS)
if st.button("🔄 ESCANEAR TENDÊNCIAS GLOBAIS E REORGANIZAR POSIÇÕES (REAL-TIME)"):
    st.info("Varrendo servidores de busca gringos e recalculando métricas de tráfego...")
    ...
    st.success("Tendências e posições do mercado recalculadas com sucesso!")
